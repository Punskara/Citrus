	...
	var init = {
			'installation.html' : function () {
				...
				$('#f_install').bind('submit', function ( e ) {
					...
					formTraitement();
					$('body > header > nav a[href=configuration.html]').removeClass('disabled').trigger('click');
			...
			'configuration.html' : function () {
				...
				$('table.t_host tbody#recept tr').live('click', function () {
					if ( !$(this).hasClass('vide') ) getHost( this.id.split('_').pop() );
				});
				// suppression d'un host
				$('table.t_host tbody#recept tr a').live('click', function (e) {
				...
				$('form#f_addHosts').bind('submit', function (e) {
					$('body > header > nav a[href=apps.html]').removeClass('disabled').trigger('click');
					e.stopImmediatePropagation();
					return false;
				});
			...
			'apps.html' : function () {
				...
				// suppression d'apps
				$('table.t_apps tbody#recept tr a').live('click', function (e) {
				...
				$('table.t_apps tbody#recept tr.app').live('click', function () {
				...
				$('table.t_modules tbody#recept tr a').live('click', function (e) {
					var id = $('td:first', $(this).parent().parent()).html(),
 ...
 */
	...
	$('div.msgbox button').live('click', function () { $(this).parents('div.msgbox').hide(); });
	...
	if (!generated) $('body > header > nav a[href=model.html]').remove();
